...
const MyPlanCard = ({ imgUrl, className, children, pTitle, pText, bText, buttonClass, pTitleClassName, bigTitle, functionCall }: myPlanCardProps) => {
  return (
    <div className={`flex mt-[96px] ${className}  max-mdp:justify-center `}>
      ...
      <div className="w-[50%] flex flex-col items-center justify-center  max-mdp:w-[50%] max-sm:w-[85%] max-mdp:h-max max-mdp:bg-white max-mdp:z-10 bg-white max-mdp:p-[25px] max-mdp:rounded-[5px] max-mdp:mt-[100px]">
        ...
        <ParagraphField
          title={pTitle ? pTitle : ''}
          paragraph={pText}
          tClassName={`text-4xl ${pTitleClassName} max-mdp:text-2xl`}
