...
const LegalTermsTemplate = () => {
    ...
    return (
        <SectionContainer innerWrapperClassName='flex justify-center pb-[200px] max-sm:px-[19px]'  >
            <div className='max-w-[580px] mt-[135px] bg-white '>
                <div className='w-full border-y-2 first-letter:first-line:border-gray-2 pt-[25px]'>
                    <TitleField text={t('title')} className='text-5xl  max-sm:text-2xl' />
                    <div className='w-full flex justify-end mb-[23px] text-gray-5'>
                        18 Jan 2019
                    </div>
                </div>
                {
                    [...new Array(5)].map((_, i) => <p className='mt-[35px]  text-lg  max-sm:text-base'>{t(`pText.${i}`)}</p>)
                }
